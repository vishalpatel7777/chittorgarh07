...
const MainboardIpoDocuments = ({ data }) => {
  return (
    <div className="relative group">
      ...
      <Swiper
        ...
        slidesPerView={3}
        onBeforeInit={(swiper) => {
          swiper.params.navigation.prevEl = ".ipo-prev";
          swiper.params.navigation.nextEl = ".ipo-next";
        }}
        navigation
        breakpoints={{
          0: { slidesPerView: 1.1 },
      ...
      >
        {data.map((item, idx) => (
          <SwiperSlide key={idx}>
            <div className="border border-emerald-600 rounded-xl p-4 bg-white h-full overflow-visible">
      ...
      <button
        className="ipo-prev absolute left-2 top-1/2 -translate-y-1/2 
      ...
      <button
        className="ipo-next absolute right-2 top-1/2 -translate-y-1/2 
                    opacity-0 group-hover:opacity-100 transition
                    bg-white border rounded-full p-2 shadow z-10"
      >
        ›
...
export default MainboardIpoDocuments;
